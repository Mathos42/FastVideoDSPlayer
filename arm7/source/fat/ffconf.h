...
/  TWiLight Menu++ (via argv) are UTF-8 encoded; with the ANSI/OEM setting,
/  file/folder names containing accented characters (é, è, à, ...) would
/  fail to match, causing f_open() to fail as if the file didn't exist.
/  TCHAR stays `char` in both modes so no other code needs to change. */
...
// this Zephyr-hardened build of FatFs R0.16 expects FS_FATFS_WINDOW_ALIGNMENT
// to be supplied externally (normally via Zephyr's own Kconfig/build system);
// since this isn't a Zephyr project, define it ourselves. 32 matches the
// cache-line-safe alignment already used elsewhere in this codebase (the
// ARM7 itself has no data cache, so the exact value isn't critical here).
#define FS_FATFS_WINDOW_ALIGNMENT 32
